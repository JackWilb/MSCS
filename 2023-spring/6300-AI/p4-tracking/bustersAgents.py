...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            most_likely_position = dist.argMax()
            distance = self.distancer.getDistance(pacmanPosition, most_likely_position)

            if distance < min_distance:
                min_distance, closest_position = distance, most_likely_position

        min_distance = float("inf")
        best_action = None
        for action in legal:
            newPos = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(newPos, closest_position)

            if distance < min_distance:
                min_distance, best_action = distance, action
