...
(define-type-alias (Envof 'a) (Listof (Bindingof 'a)))
...
;; parse ----------------------------------------
(define parse : (S-Exp (S-Exp -> 'a) S-Exp -> (Expof 'a))
  (lambda (s convert match)
    (cond
      [(s-exp-match? match s) (litE (convert s))]
      [(s-exp-match? `SYMBOL s) (idE (s-exp->symbol s))]
      [(s-exp-match? `{+ ANY ANY} s)
       (plusE (parse (second (s-exp->list s)) convert match)
              (parse (third (s-exp->list s)) convert match))]
      [(s-exp-match? `{* ANY ANY} s)
       (multE (parse (second (s-exp->list s)) convert match)
              (parse (third (s-exp->list s)) convert match))]
      [(s-exp-match? `{let {[SYMBOL ANY]} ANY} s)
       (let ([bs (s-exp->list (first
         ...
         (appE (lamE (s-exp->symbol (first bs))
                     (parse (third (s-exp->list s)) convert match))
               (parse (second bs) convert match)))]
      [(s-exp-match? `{lambda {SYMBOL} ANY} s)
       (lamE (s-exp->symbol (first (s-exp->list 
                                    (second (s-exp->list s)))))
             (parse (third (s-exp->list s)) convert match))]
      [(s-exp-match? `{ANY ANY} s)
       (appE (parse (first (s-exp->list s)) convert match)
             (parse (second (s-exp->list s)) convert match))]
      [else (error 'parse "invalid input")])))

(define (parse/num [s : S-Exp]) : (Expof Number)
  (parse s s-exp->number `NUMBER))

(define (parse/str [s : S-Exp]) : (Expof String)
  (parse s s-exp->string `STRING))
...
;; interp ----------------------------------------
(define interp : ((Expof 'a) (Envof 'a) ((Valueof 'a) (Valueof 'a) -> (Valueof 'a)) ((Valueof 'a) (Valueof 'a) -> (Valueof 'a)) -> (Valueof 'a))
  (lambda (a env plus times)
    (type-case (Expof 'a) a
      [(litE n) (litV n)]
      [(idE s) (lookup s env)]
      [(plusE l r) (plus (interp l env plus times) (interp r env plus times))]
      [(multE l r) (times (interp l env plus times) (interp r env plus times))]
      [(lamE n body)
       (closV n body env)]
      [(appE fun arg) (type-case (Valueof 'a) (interp fun env plus times)
                        [(closV n body c-env)
                         (interp body
                                 (extend-env
                                  (bind n
                                        (interp arg env plus times))
                                  c-env)
                                 plus times)]
                        [else (error 'interp "not a function")])])))

(define (interp/num [a : (Expof Number)] [env : (Envof Number)]) : (Valueof Number)
  (interp a env num+ num*))

(define (interp/str [a : (Expof String)] [env : (Envof String)]) : (Valueof String)
...
(define (num+ [l : (Valueof Number)] [r : (Valueof Number)]) : (Valueof Number)
  (num-op + l r))
(define (num* [l : (Valueof Number)] [r : (Valueof Number)]) : (Valueof Number)
  (num-op * l r))
...
(define (str+ [l : (Valueof String)] [r : (Valueof String)]) : (Valueof String)
...
;; lookup ----------------------------------------
(define lookup : (Symbol (Envof 'a) -> (Valueof 'a))
  (lambda (n env)
    (cond
