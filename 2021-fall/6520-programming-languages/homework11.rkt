...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(restE a) (type-case Value (interp a env)
                 ...
                 [else (error 'interp "not a list")])]
    [(if0E tst thn els)
     (if (equal? (numV 0) (interp tst env))
         (interp thn env)
         (interp els env))]))

(module+ test
  (test (interp (parse `{if0 0 2 3}) mt-env)
        (numV 2))
  (test (interp (parse `{if0 1 2 3}) mt-env)
        (numV 3))
  ...
  (type-case Exp a
    ...
    [(emptyE)
     (local [(define result-type (varT (box (none))))]
       (listofT result-type))]
    [(consE l r)
     (local [(define result-type (listofT (varT (box (none)))))]
          (begin
            (unify! (typecheck r tenv)
                    result-type
                    r)
            (unify! (typecheck l tenv)
                    (listofT-elem result-type)
                    l)
            result-type))]
    [(firstE a)
     (local [(define result-type (listofT (varT (box (none)))))]
    ...
    [(if0E tst thn els)
     (begin
       (unify! (typecheck tst tenv) (numT) tst)
       (let ([thn-t (typecheck thn tenv)])
         (begin
           (unify! thn-t (typecheck els tenv) els)
           thn-t)))]))
...
(module+ test
  ...
  (test (typecheck (parse `{cons {lambda {[x : num]} 12} empty}) mt-env)
        (listofT (varT (box (some (arrowT (numT) (numT)))))))
  (test (typecheck (parse `{cons 1 empty}) mt-env)
        (listofT (varT (box (some (numT))))))
  (test (typecheck (parse `{cons empty empty}) mt-env)
        (listofT (varT (box (some (listofT (varT (box (none)))))))))

  (test (typecheck (parse `{first {cons 1 empty}}) mt-env)
        (varT (box (some (numT)))))
  (test (typecheck (parse `{rest {cons 1 empty}}) mt-env)
        (listofT (varT (box (some (numT))))))
...
(define (run-prog [input : S-Exp]) : S-Exp
