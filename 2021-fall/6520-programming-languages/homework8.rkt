...
(define (compile a-fae env)
  (type-case Exp a-fae
    ...
    [(boxE v) (code-malloc1 18 (compile v env))]
    [(unboxE b) (code-malloc1 19 (compile b env))]
    [(box?E b) (code-malloc1 20 (compile b env))]))
  ...
  (if (= 99 (vector-ref from-memory n))
      ...
      (begin
        (case (vector-ref from-memory n)
          ...
          [else
           (error 'move "internal error: unknown tag")])
        ;; Change to gray:
  ...
  (case (ref k-reg 0)
    ...
    [(22) ; doBoxK
     (begin
       ...
       (set! v-reg (malloc1 21 (ref v-reg 1)))
       (continue))]
    [(23) ; doUnboxK
     (begin
       (set! k-reg (ref k-reg 1))
    ...
    [(24) ;doBox?K
     (begin
       (if (equal? (ref v-reg 0) 21)
           (set! v-reg (malloc1 15 0))
           (set! v-reg (malloc1 15 1)))
       (set! k-reg (ref k-reg 1))
       (continue))]
    [else
...
(module+ test
  ...
  ;; for test coverage:
  (reset!)
  (test/exn (interpx (code-malloc1 99 0) empty-env (init-k))
            "internal error")
  (reset!)
  (test/exn (interpx (compile (parse `1) mt-env)
                     empty-env
                     (code-malloc1 99 0))
            "internal error")
  (reset!)
